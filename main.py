        ...
        for i in range(max_label + 1):
            ...
            if len(cluster_idx) > p1_cfg["min_cluster_points"]:
                ...
                if len(sub_cloud.points) > 0:
                    ...
                    dim_min = cfg_det["table_dimensions"]["min_bounds"]
                    dim_max = cfg_det["table_dimensions"]["max_bounds"]
                    
                    if (dim_min[0] < dims[0] < dim_max[0]) and \
                       (dim_min[1] < dims[1] < dim_max[1]) and \
                       (dim_min[2] < dims[2] < dim_max[2]):
